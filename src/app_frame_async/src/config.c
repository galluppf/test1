...
/*    dma_pipeline.row_size_max = sizeof(synaptic_row_t) + sizeof(uint) * app_data.synaptic_row_length;*/
    ...
    // get correct address for population data
    population->neuron = (void *) &population[app_data.num_populations];
...
{ //TODO clean up this horific mess of addresses and offsets
    ...
    if (app_data.num_populations > 1) {
      io_printf(IO_STD, "Error detected, each core should have only one population.\n");
      io_printf(IO_STD, "  This core is asked to simulate %d populations instead.\n", app_data.num_populations);
      while(1);
    }

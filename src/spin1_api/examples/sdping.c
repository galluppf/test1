...
// Port 1 - print it using printf & and return to sender
...
void process_sdp (uint m, uint port)
{
  ...
  if (port == 1) // Port 1 - echo to sender
    {
      swap_sdp_hdr (msg);
      (void) spin1_send_sdp_msg (msg, 1000);
    }
  else if (port == 2) // Port 2 - send via IPTAG 1
    {
      ...
      msg->srce_port = virt_cpu;	// Source port
      msg->srce_addr = sv->p2p_addr;	// Source addr
